...
打开文件夹之后插件会到`.vscode`文件夹下寻找配置文件，如果找不到会建议你配置`property.json`，同时会检索当前文件夹下所有的HDL文件，并进行分析。
...
使用快捷键`F1`呼出控制命令框，输入 *Digital-IDE:Generate property.json* 即可生成初始的 `property.json` 模板文件。生成的配置文件会直接放置于.vscode的文件夹下

如果你有属于自己的模板可以使用*Digital-IDE:Overwrite property.json template* 来自定义模板文件。

如果你是0.3.0之前的工程配置文件的话，可以输入*Digital-IDE: Transform configure file from previous version to new version*来进行配置文件的更新。
